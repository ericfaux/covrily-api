...
// Vercel function runtime (must be "nodejs" – not "nodejs18.x")
...
function verifySignature(req: VercelRequest): boolean | "skipped" {
  ...
  // We don’t get a rawBody on Node runtime; use the parsed body.
...
function extractUserId(toFull: PMAddress[] | undefined): string | null {
  for (const r of toFull || []) {
    ...
    // Accept plus-tag (…+<uuid>@…) or MailboxHash
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Signature check (allow skip while testing if ALLOW_UNVERIFIED_INBOUND=true)
  ...
  if (!userId) {
    return res
      .status(400)
      .json({ ok: false, error: "user id not resolved (missing +<uuid> tag and no default set)" });
  }
  ...
  try {
    ...
    // Basic merchant inference; your H&M PDFs will be parsed by parseHmPdf
    const fromDomain = ((payload.From || "").split("@")[1] || "").toLowerCase();
    const merchant = fromDomain.includes("hm.") ? "hm.com" : fromDomain || "unknown";

    // Parse the PDF (H&M-specific light parser you added in lib/pdf.ts)
      ...
      .insert([
        {
          ...
          purchase_date:
            preview.order_date || preview.receipt_date || new Date().toISOString(),
        },
      ])
      .select()
      .single();
    ...
    // Save the PDF to Storage (best-effort)
    let storage_path: string | null = null;
    try {
      storage_path = await uploadPdf(supabase, userId, ins.id, att.Name || "receipt.pdf", pdfBuf);
    } catch {
